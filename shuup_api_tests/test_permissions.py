...
def test_api_permissions_anonymous():
    ...
    assert client.post("/api/test/user/", {"email": "xiloca@djsij.com"}).status_code == status.HTTP_401_UNAUTHORIZED
...
def test_api_permissions_authenticated_user():
    ...
    assert client.post("/api/test/user/", {"email": "xiloca@djsij.com"}).status_code == status.HTTP_403_FORBIDDEN
